...
begin
  ...
  --
  -- Every check writes exactly one line, starting with "ok " or "FAIL ". The
  -- log is split into lines and each line is classified on its own: a line
  -- that starts with neither counts as a failure, so a mistyped prefix cannot
  -- hide a check. The total is then compared with the number of checks this
  -- file contains, so a branch that writes nothing, a check that never ran, or
  -- a NULL that wiped the log is reported as a failure instead of passing.
  --
  -- Adding or removing a check means updating c_expected_checks.

  declare
    c_expected_checks constant integer := 35;
